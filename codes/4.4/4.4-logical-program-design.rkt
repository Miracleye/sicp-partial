  ...
  (let ((local-table (list '*table*)))
    ...
    (define (dispatch m)
      (cond ((eq? m 'lookup-proc) lookup)
            ...
            (else (error "Unknown operation -- TABLE-2" m))))
      dispatch))
...
;;; init the database
(define db (open-input-file "simple-db.txt"))
(define (iter-read)
  (let ((record (read db)))
    (if (eof-object? record)
        'eof
        (begin (add-rule-or-assertion! record)
               (iter-read)))))
(iter-read)