...
(define (make-procedure parameters body env)
  (list 'procedure parameters
        (let ((exp (scan-out-defines body)))
          (if (let? exp)
              (cons (let->combination exp) '())
              exp))
        env))
...
(define (scan-out-defines proc-body)
  ...
  (define (non-define-clauses proc-body)
    (if (definition? (car proc-body))
        (non-define-clauses (cdr proc-body))
        proc-body))
  ;; from here
  (let ((definitions (filter definition? proc-body)))
    (if (null? definitions)
        ...
        (let ((let-clauses (map (lambda (def)
                                  (cons (definition-variable def) (cons ''*unassigned* '())))  ;; here are two '
                                definitions))
              (assignments (map (lambda (def)
                                  (list 'set! (definition-variable def) (definition-value def)))
                                definitions))
              (exps (non-define-clauses proc-body)))
